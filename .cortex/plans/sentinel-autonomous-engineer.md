...
Updated-by:
  ...
  - 2026-04-28 claude-code (slimmed to cross-cutting only per house rule: per-tool work → GitHub issues, plans-in-tool-repos)
Cites:
  - doctrine/0001-why-autumn-garage-exists
  - doctrine/0003-llm-providers-compose-by-contract
  - doctrine/0004-conductor-as-fourth-peer
---
...
>
> **Scope of this document.** This is autumn-garage's coordination plan. It covers cross-tool product framing, the four contracts that bind the four tools together, and the cross-cutting decisions resolved through 2026-04-28 deep research. Per-tool implementation work lives as GitHub issues against the respective tool repos (`autumngarage/sentinel`, `autumngarage/cortex`, `autumngarage/conductor`, `autumngarage/touchstone`); deeper tool-specific plans live in those tools' own `.cortex/plans/`.
...
### 1. Default Doctrine pack — ships in Sentinel, seeded by `sentinel init`

**Decision:** Sentinel ships engineering-values Doctrine in `~/repos/sentinel/defaults/doctrine/`. `sentinel init` copies into the project's `.cortex/doctrine/` if that directory is empty or absent, with sequential numbering preserved (0001–0015) and a `Sentinel-baseline: true` frontmatter flag so projects can distinguish defaults from local entries. Projects supersede via Cortex's existing immutable-with-supersede mechanism.
...
**Decision:** Sentinel calls Conductor via `conductor call ...` subprocess, matching Touchstone's pattern. Drop the `conductor @ git+...@v0.3.3` git-ref pin in `pyproject.toml`. The existing `sentinel-conductor-migration` plan's Slice B (`ConductorAdapter` shim) keeps its shape — it just wraps subprocess instead of `import`.
...
Implementation work lives in each tool's repo as GitHub issues (or in-repo plans for larger workstreams). The autumn-garage plan does not track per-tool task lists — see the GitHub project board for status. Issue families to file against each repo:

- **`autumngarage/sentinel`** — `init` bootstrap; read-side Cortex consumption; rejection-log fold into Cortex Journal; per-task file-state isolation; cycle-artifact schema implementation; trust controls (loop detection, destructive-change gate); scheduled `sentinel work` mode. Plus continuation of the conductor-seam migration in sentinel's own `.cortex/plans/sentinel-conductor-migration.md` (moved 2026-04-28; Slice B revised for subprocess seam per cross-cutting decision 4). Wave 1 issues filed: sentinel#89 (subprocess migration), sentinel#90 (default Doctrine pack).
- **`autumngarage/cortex`** — default-Doctrine seeding flow on `cortex init` (the *mechanism* — Sentinel ships the *content*); audit `cortex grep --frontmatter` filter coverage for Sentinel's queries; coordinate Phase D `cortex journal append` CLI with Sentinel's later workstream. Wave 1 issues filed: cortex#60 (grep filter audit), cortex#61 (`--seed-from` flag).
- **`autumngarage/conductor`** — audit subprocess CLI surface for Sentinel's per-role needs (gaps = small Conductor PRs filed during the migration); regression test asserting `conductor call`'s flag surface and JSON output schema; `consumers.md` documentation under README. Wave 1 issue filed: conductor#93.
...
All four cross-cutting questions resolved above. Meta-question about plan placement also resolved 2026-04-28: `sentinel-conductor-migration.md` moved to `~/repos/sentinel/.cortex/plans/` with Slice B and Key design decisions revised for the subprocess seam.
